...
class Meter(Frame):
    ...
    def draw_base_image(self):
        ...
        if self.stripethickness > 0:
            for x in range(
                self.arcoffset, self.arcrange + self.arcoffset, 2 if self.stripethickness == 1 else self.stripethickness
            ):
                draw.arc(
                    (0, 0, self.metersize * 5 - 20, self.metersize * 5 - 20),
                    x,
                    x + self.stripethickness - 1,
                    self.meterbackground,
...
if __name__ == "__main__":
    ...
    Meter(metersize=180, padding=20, amountused=25, metertype="semi", labeltext="miles per hour", interactive=True).grid(
        row=0, column=0
    )
    Meter(
        metersize=180,
